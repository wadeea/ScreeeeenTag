...
export default function TagFleetView() {
  const { tags, products, fetchData, isLoading } = useStore();
  ...
  async function handleBind(tag_id: string, sku: string) {
    try {
      const res = await api.tags.bind(tag_id, sku);
      if (res.success) {
        setIsBinding(false);
        setSelectedTag(null);
        // In Phase 3, we won't need manual fetchData here as sockets will update us
        fetchData();
      }
    } catch (err) {
      console.error(err);
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-slate-100 font-mono text-sm font-bold tracking-tight">{tag.id}</h3>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center gap-1.5">
                        <span className={cn(
                          "text-[10px] font-mono",
                          tag.battery_level > 50 ? "text-emerald-400" : tag.battery_level > 20 ? "text-amber-400" : "text-red-400"
                  ...
                  <div className="bg-black/20 rounded-lg p-3 border border-white/5 mb-0">
                    <div className="flex gap-3 items-center">
                      <div className="flex-1">
                        {tag.product_sku ? (
                          <>
                            <p className="text-xs font-sans text-slate-300 line-clamp-1">{tag.product_name}</p>
                            <div className="flex items-center justify-between mt-1">
                              <p className="text-xs font-mono font-bold text-sky-400">${tag.product_price?.toFixed(2)}</p>
                              <p className="text-[9px] font-mono text-slate-500">{tag.product_sku}</p>
                            </div>
                          </>
                        ) : (
                      ...
                      <div className={cn(
                        ...
                        tag.product_sku ? "bg-sky-500/10 text-sky-400" : "bg-white/5 text-slate-600"
                      )}>
                        {tag.product_sku ? <Link2 className="w-4 h-4" /> : <Link2Off className="w-4 h-4" />}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="col-span-1">
          {selectedTag ? (
            <div className="glass-card rounded-xl p-6 sticky top-24 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-white/5">
                  ...
                  <span className="text-sm text-slate-100 font-mono font-bold tracking-tight">{selectedTag.id}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-white/5">
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Binding</span>
                  <span className="text-sm text-sky-400 font-mono uppercase">{selectedTag.product_sku || 'None'}</span>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-4">
                ...
                >
                  ...
                  {isBinding ? 'Cancel Binding' : 'Bind New Product'}
                </button>
              ...
              {isBinding && (
                <div className="mt-6 pt-6 border-t border-white/10 animate-in fade-in slide-in-from-top-4 duration-300">
                  <p className="text-[10px] text-slate-400 font-bold mb-3 uppercase tracking-widest">Product Catalog</p>
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                    {filteredProducts.map(p => (
                      <button 
                        key={p.id}
                        onClick={() => handleBind(selectedTag.id, p.sku)}
                      ...
                      >
                        <div className="overflow-hidden">
                          <p className="text-xs font-bold text-slate-200 truncate">{p.sku}</p>
                          <p className="text-[10px] text-slate-500 truncate">{p.name}</p>
                        </div>
                        <ChevronRight className="w-4 h-4 text-slate-700 group-hover:text-sky-400 transition-colors shrink-0" />
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="glass-card rounded-xl p-12 text-center h-full flex flex-col items-center justify-center border-dashed border-white/20">
              ...
              <p className="text-slate-500 text-xs font-medium uppercase tracking-widest leading-relaxed">System Ready<br/>Select a Node to Configure</p>
            </div>
          )}
        </div>
      </div>
